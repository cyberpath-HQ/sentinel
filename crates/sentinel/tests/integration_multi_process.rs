...
async fn test_multi_process_concurrent_inserts() {
    ...
    for i in 0 .. 3 {
        ...
        let child = std::thread::spawn(move || {
            ...
            let collection = tokio::runtime::Runtime::new()
                .unwrap()
                .block_on(async { store.collection_with_config("test_collection", None).await })
                .unwrap();
    ...
    let store = tokio::runtime::Runtime::new()
        .unwrap()
        .block_on(async { Store::new_with_config(&store_path, None, sentinel_wal::StoreWalConfig::default()).await });
