...
```typescript
import { SentinelStore, SentinelCollection, SentinelDocument } from "@cyberpath/sentinel";

const store = await SentinelStore.create("./my-database", null);
...
```javascript
const { SentinelStore, QueryBuilder, Operator } = require("@cyberpath/sentinel");
...
async function main() {
  ...
  const query = new QueryBuilder()
    .filter("age", Operator.GreaterThan, 26)
...
| ------------------------ | ---------------- | ------------------------------------------------------------ |
| `Operator.Equals`        | Exact match      | `.filter('role', Operator.Equals, 'admin')`                  |
| `Operator.NotEquals`     | Inequality       | `.filter('active', Operator.NotEquals, false)`               |
| `Operator.GreaterThan`   | > comparison     | `.filter('age', Operator.GreaterThan, 25)`                   |
| `Operator.LessThan`      | < comparison     | `.filter('age', Operator.LessThan, 30)`                      |
| `Operator.GreaterOrEqual`| >= comparison    | `.filter('age', Operator.GreaterOrEqual, 25)`                 |
| `Operator.LessOrEqual`   | &lt;= comparison | `.filter('age', Operator.LessOrEqual, 30)`                   |
| `Operator.Contains`      | String contains  | `.filter('name', Operator.Contains, 'Al')`                   |
| `Operator.StartsWith`    | String prefix    | `.filter('email', Operator.StartsWith, 'admin')`             |
| `Operator.EndsWith`      | String suffix    | `.filter('email', Operator.EndsWith, '@company.com')`        |
| `Operator.In`            | Value in array   | `.filter('role', Operator.In, ['admin', 'moderator'])`       |
| `Operator.Exists`        | Field exists     | `.filter('phone', Operator.Exists, true)`                    |
...
async function main() {
  ...
  const query = new QueryBuilder()
    .filter("role", Operator.Equals, "admin")
    .filter("active", Operator.Equals, true)
...
async function main() {
  ...
  const store = await SentinelStore.create("./secure-database", "my-secret-passphrase");
  const users = await store.collection("users");
...
async function main() {
  ...
  const store = await SentinelStore.create("./encrypted-database", "secure-passphrase");
  const users = await store.collection("users");
...
async function main() {
  try {
const store = await SentinelSentinelStore.create("./my-database");
    const users = await store.collection("users");
...
async function initialize() {
  store = await SentinelStore.create("./data", null);
}
...
const store = await SentinelStore.create(path.resolve("./data"), null);
```
