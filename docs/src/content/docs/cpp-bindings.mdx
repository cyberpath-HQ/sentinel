...
```cpp
#include <sentinel-cxx/sentinel-cxx.hpp>
#include <iostream>
...
int main() {
    ...
    // Insert document
    sentinel_document_t* doc = sentinel_collection_insert(users, "alice",
        "{\"name\": \"Alice Johnson\", \"email\": \"alice@example.com\"}");

    // Get document
    doc = sentinel_collection_get(users, "alice");
    if (doc) {
        const char* data = sentinel_document_data(doc);
        printf("Found: %s\n", data);
        sentinel_document_destroy(doc);
    }
