---
title: Store
description: Learn about Store, the top-level container for collections in Sentinel.
...
instance backed by a directory on your filesystem. This guide covers creating stores, configuring signing, listing and
deleting collections, and understanding the directory structure.
...
in a special collection that only the passphrase can unlock.
...
This method returns a vector of collection names in no particular order. If the store has no collections, an empty vector
is returned.
...
The deletion operation is permanent and cannot be undone. If the collection doesn't exist, the operation succeeds
silently (idempotent). Use with caution.
...
This approach is useful when you want to manage keys separately from passphrases, such as when integrating with external
...
1. **First Run**: Generates a random 32-byte salt, derives an encryption key from your passphrase using the configured
   key derivation function, generates a new Ed25519 signing key, encrypts the signing key with the derived encryption key,
   and stores the encrypted key and salt in `.keys/signing_key.json`.
...
| No leading dots          | Names starting with `.` are forbidden (except `.keys`) |
