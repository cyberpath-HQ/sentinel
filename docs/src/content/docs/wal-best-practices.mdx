...
let mut audit_config = CollectionWalConfig::default();
audit_config.write_mode = WalFailureMode::Strict;
let audit_logs_collection = store.collection_with_config("audit_logs", Some(audit_config)).await?;

// Non-critical metrics - availability more important
let mut metrics_config = CollectionWalConfig::default();
metrics_config.write_mode = WalFailureMode::Warn;
let metrics_collection = store.collection_with_config("metrics", Some(metrics_config)).await?;

// Temporary cache - no durability needed
let mut cache_config = CollectionWalConfig::default();
cache_config.write_mode = WalFailureMode::Disabled;
let cache_collection = store.collection_with_config("cache", Some(cache_config)).await?;
```
