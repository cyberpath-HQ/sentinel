...
async fn verification_loop(collection: Collection) {
    ...
    loop {
        ...
        match collection.verify_wal().await {
            Ok(result) => {
                if result.passed {
...
pub async fn safe_recovery(collection: &Collection) -> Result<()> {
    ...
    let re_verification = collection.verify_wal().await?;
...
pub async fn check_wal_health(collection: &Collection) -> WalHealthReport {
    let result = collection.verify_wal().await.ok();
...
tokio::spawn({
    ...
    async move {
        let _ = collection.verify_wal().await;
    }
});
```
