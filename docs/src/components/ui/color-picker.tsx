...
function parseColorString(value: string): ColorValue | null {
    ...
    if (trimmed.startsWith(`#`)) {
        const hexMatch = /^#([a-fA-F0-9]{3}|[a-fA-F0-9]{6})$/.exec(trimmed);
        if (hexMatch) {
    ...
    const hsbMatch = /^hsba?\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([\d.]+))?\s*\)$/.exec(trimmed);
    if (hsbMatch) {
...
function ColorPickerImpl(props: ColorPickerImplProps) {
    ...
    const [
        formTrigger,
        setFormTrigger,
    ] = React.useState<RootElement | null>(
        null
    );
...
function ColorPickerContent(
    ...
    const {
        asChild, className, children, ...popoverContentProps
    } = props;
...
function ColorPickerEyeDropper(props: React.ComponentProps<typeof Button>) {
    const {
        size: sizeProp, children, disabled, ...buttonProps
    } = props;
...
function ColorPickerFormatSelect(props: ColorPickerFormatSelectProps) {
    const {
        size, disabled, className, ...selectProps
    } = props;
...
function RgbInput(props: FormatInputProps) {
    ...
    const onChannelChange = React.useCallback(
        (channel: `r` | `g` | `b` | `a`, max: number, isAlpha = false) => (event: React.ChangeEvent<InputElement>) => {
            const value = Number.parseInt(event.target.value, 10);
